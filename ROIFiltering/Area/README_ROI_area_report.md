# ROI Area Report Script

## Purpose

This script produces a multi-channel ROI shape report ranked specifically by **Area**.

It reads ROI pixel-coordinate CSVs and their corresponding 2-D fluorescence TIFFs, reconstructs ROI geometry directly from the pixel coordinates, calculates a full set of shape measurements, ranks ROIs by `Area_um2`, divides them into 5%-wide rank-based percentile groups, randomly samples example ROIs from each group, and produces detailed PDF and CSV outputs.
...
```text
/home/oltij/Desktop/ROI_area_report
```
...
## Focal ranking metric — Area

The focal ranking metric is physical ROI area:
...
With the current scale, each ROI pixel contributes approximately:

```text
0.312843137 × 0.312836345 µm²
```
...
│   └── Area/
│       ├── PV_Area_report.pdf
│       ├── PV_Area_selected_ROIs.csv
│       └── PV_Area_all_ROIs.csv
├── Hoescht/
│   ├── Hoescht_all_ROI_measurements.csv
│   └── Area/
│       ├── Hoescht_Area_report.pdf
│       ├── Hoescht_Area_selected_ROIs.csv
│       └── Hoescht_Area_all_ROIs.csv
└── LHX6/
    ├── LHX6_all_ROI_measurements.csv
    └── Area/
        ├── LHX6_Area_report.pdf
        ├── LHX6_Area_selected_ROIs.csv
        └── LHX6_Area_all_ROIs.csv
```

Because `Area` contains no spaces, the metric folder and filename component are exactly `Area`.
...
```text
<Channel>/Area/<Channel>_Area_report.pdf
```

For example:

```text
PV/Area/PV_Area_report.pdf
```
...
```text
PV/Area/PV_Area_selected_ROIs.csv
```
...
```bash
python your_area_report_script.py
```
...
ROI Area report script
        ...
        +--> per-channel Area PDF
        +--> per-channel Area all-ROI CSV
        +--> per-channel Area selected-ROI CSV
```
