...
output_dir = Path("/home/oltij/Desktop/ROI_eccentricity_report")
...
channels = [
    ...
    },
    {
        "name": "Hoescht",
        "roi_csv": (
            "/home/oltij/Desktop/ROI_intensitymetric_filtering/Hoescht/Hoescht_ROI_pixels_FILTERED.csv"
        ),
        "original": (
...
# IMAGE PIXEL SIZE
# ------------------------------------------------------------
# Physical pixel dimensions from the original IMS metadata.
# X and Y are kept separate because the pixels are slightly anisotropic.
...
# Image display percentiles for grayscale images
...
pdf_path = output_dir / "All_Channel_ROI_Shape_Report.pdf"
csv_output_path = output_dir / "All_Channel_ROI_Shape_Selected.csv"
all_measurements_csv = output_dir / "All_Channel_ROI_Shape_AllObjects.csv"
...
# The full set of shape measurements is calculated for every ROI.
# Each of the four scripts below ranks by one of these metrics.
SHAPE_METRICS = {
    "Eccentricity": None,
}

# Human-readable name used for folders and PDF labels.
METRIC_DISPLAY_NAME = "Eccentricity"
...
def draw_filled_mask_outline(ax, mask, color, linewidth):
    ...
    Contouring the filled mask (rather than a boundary mask) prevents the
    double-line artifact caused by contouring both sides of a boundary strip.
    """
    if mask.any():
        ax.contour(
            mask.astype(float),
...
def calculate_roi_measurements(roi_pixels, image):
    ...
    extent = float(n_pixels / bbox_area) if bbox_area > 0 else np.nan

    # ------------------------------------------------------------
    # Binary ROI mask, local to this object only
    # ------------------------------------------------------------
    mask = np.zeros((bbox_height, bbox_width), dtype=bool)
    local_x = xs - xmin
    local_y = ys - ymin
    mask[local_y, local_x] = True

    # ------------------------------------------------------------
    # Anisotropy-aware perimeter
    # ------------------------------------------------------------
    # Top/bottom exposed edges span X; left/right exposed edges span Y.
    ...
    top_edges = int(np.sum(mask & ~up))
    bottom_edges = int(np.sum(mask & ~down))
    left_edges = int(np.sum(mask & ~left))
    right_edges = int(np.sum(mask & ~right))

    perimeter_um = float(
        (top_edges + bottom_edges) * PIXEL_SIZE_X_UM
        + (left_edges + right_edges) * PIXEL_SIZE_Y_UM
    )
    ...
    points_um = np.column_stack([
        xs.astype(np.float64) * PIXEL_SIZE_X_UM,
        ys.astype(np.float64) * PIXEL_SIZE_Y_UM,
    ])

    convex_hull_area_um2 = np.nan
    if n_pixels >= 3:
        try:
            hull = ConvexHull(points_um)
            convex_hull_area_um2 = float(hull.volume)
        except QhullError:
            convex_hull_area_um2 = np.nan

    if np.isfinite(convex_hull_area_um2) and convex_hull_area_um2 > 0:
        solidity = float(area_um2 / convex_hull_area_um2)
    else:
        solidity = 1.0 if n_pixels > 0 else np.nan

    # ------------------------------------------------------------
    # Eccentricity / axes from physical-coordinate second moments
    # ------------------------------------------------------------
    eccentricity = np.nan
    ...
    if n_pixels >= 2:
        ...
        covariance = (centered.T @ centered) / n_pixels
        try:
            ...
            if lambda_major > 0:
                eccentricity = float(
                    math.sqrt(max(0.0, 1.0 - lambda_minor / lambda_major))
                )
                major_axis_length_um = float(4.0 * math.sqrt(lambda_major))
                minor_axis_length_um = float(4.0 * math.sqrt(lambda_minor))
    ...
    return {
        ...
        "ConvexHullArea_um2": convex_hull_area_um2,
        "MajorAxisLength_um": major_axis_length_um,
        "MinorAxisLength_um": minor_axis_length_um,
        "AspectRatio": aspect_ratio,
        "EquivalentDiameter_um": equivalent_diameter_um,
        "Extent": extent,
        "ROI pixel count": n_pixels,
    }
...
with PdfPages(pdf_path) as pdf:
    ...
    for channel_number, channel_info in enumerate(channels, start=1):
        ...
        # --------------------------------------------------------
        # CALCULATE SHAPE METRICS FROM ROI PIXELS
        # --------------------------------------------------------
        measurement_rows = []

        print("\nCalculating shape metrics directly from ROI pixels...")
        ...
        # --------------------------------------------------------
        # PROCESS RANKING METRIC
        # --------------------------------------------------------
        for metric_number, metric_name in enumerate(SHAPE_METRICS, start=1):
            metric_column_name = metric_name
            metric_name = METRIC_DISPLAY_NAME
            print("\n" + "-" * 70)
            print(f"{channel_name} | {metric_number}/{len(SHAPE_METRICS)}: {metric_name}")
            print("-" * 70)
            ...
            temp[metric_column_name] = pd.to_numeric(temp[metric_column_name], errors="coerce")
            temp = temp.dropna(subset=[metric_column_name])
            temp = temp[temp[metric_column_name] >= 0].copy()

            values = temp[metric_column_name].to_numpy()
            ...
            sorted_temp = temp.sort_values(
                by=[metric_column_name, "ObjectID"],
                ascending=[True, True],
            ...
            # Attach percentile information to the complete ROI measurement table
            # so the all-ROI CSV records the percentile bin for every ROI.
            # The percentile columns live on the individual bin dataframes,
            # not on sorted_temp itself. Combine those bins to build a complete
            # ObjectID -> percentile lookup for the all-ROI table.
            percentile_lookup = pd.concat(
                percentile_bin_dfs,
                ignore_index=True,
            )[[
                "ObjectID",
            ...
            for bin_index, bin_df in enumerate(percentile_bin_dfs):
                ...
                selected["Metric"] = METRIC_DISPLAY_NAME
            ...
            for page_start in range(0, N_PERCENTILE_BINS, 4):
                ...
                for panel_pos, bin_index in enumerate(page_bins):
                    ...
                    # Selected ROIs are blue only. Remove their actual
                    # boundary from the yellow layer first so no yellow remains underneath.
                    ...
                    draw_outline(
                        ax, other_outline, ALL_ROI_COLOR, ROI_LINEWIDTH
                    )
                    draw_filled_mask_outline(
                        ax, selected_mask, SELECTED_ROI_COLOR, SELECTED_ROI_LINEWIDTH
                    )
            ...
            for crop_page in range(n_crop_pages):
                ...
                for plot_idx, (_, row) in enumerate(page_df.iterrows()):
                    ...
                    # Draw all other ROI boundaries in yellow, excluding
                    # the target ROI's actual boundary.
                    selected_crop_mask = (
                    ...
                    ax.set_title(
                        ...
                        f"{metric_name}: {float(row[metric_column_name]):.4g}",
                        fontsize=8.0,
                    )
                    ax.axis("off")
...
print("DONE!")
print("=" * 70)
print("\nSingle PDF containing all channels for this shape metric:")
print(pdf_path)
