# ROI Eccentricity Report Script

## Purpose

This script produces a multi-channel ROI shape report ranked specifically by **Eccentricity**.

It reads ROI pixel-coordinate CSVs and their corresponding 2-D fluorescence TIFFs, reconstructs ROI geometry directly from the pixel coordinates, calculates a full set of shape measurements, ranks ROIs by `Eccentricity`, divides them into 5%-wide rank-based percentile groups, randomly samples example ROIs from each group, and produces detailed PDF and CSV outputs.
...
```text
/home/oltij/Desktop/ROI_eccentricity_report
```
...
## Focal ranking metric — Eccentricity

The focal ranking metric is derived from the second moments of physical-coordinate ROI pixels:
...
where `lambda_minor` and `lambda_major` are the eigenvalues of the covariance matrix.

Conceptually:

```text
0 -> isotropic / circle-like
closer to 1 -> increasingly elongated
```

Degenerate objects can have `NaN`.
...
│   └── Eccentricity/
│       ├── PV_Eccentricity_report.pdf
│       ├── PV_Eccentricity_selected_ROIs.csv
│       └── PV_Eccentricity_all_ROIs.csv
├── Hoescht/
│   ├── Hoescht_all_ROI_measurements.csv
│   └── Eccentricity/
│       ├── Hoescht_Eccentricity_report.pdf
│       ├── Hoescht_Eccentricity_selected_ROIs.csv
│       └── Hoescht_Eccentricity_all_ROIs.csv
└── LHX6/
    ├── LHX6_all_ROI_measurements.csv
    └── Eccentricity/
        ├── LHX6_Eccentricity_report.pdf
        ├── LHX6_Eccentricity_selected_ROIs.csv
        └── LHX6_Eccentricity_all_ROIs.csv
```

Because `Eccentricity` contains no spaces, the metric folder and filename component are exactly `Eccentricity`.
...
```text
<Channel>/Eccentricity/<Channel>_Eccentricity_report.pdf
```

For example:

```text
PV/Eccentricity/PV_Eccentricity_report.pdf
```
...
```text
PV/Eccentricity/PV_Eccentricity_all_ROIs.csv
```
...
```text
PV/Eccentricity/PV_Eccentricity_selected_ROIs.csv
```
...
```bash
python your_eccentricity_report_script.py
```
...
ROI Eccentricity report script
        ...
        +--> per-channel Eccentricity PDF
        +--> per-channel Eccentricity all-ROI CSV
        +--> per-channel Eccentricity selected-ROI CSV
```
