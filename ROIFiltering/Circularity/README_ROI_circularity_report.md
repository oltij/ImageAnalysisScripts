# ROI Circularity Report Script

## Purpose

This script produces a multi-channel ROI shape report ranked specifically by **Circularity**.

It reads ROI pixel-coordinate CSVs and their corresponding 2-D fluorescence TIFFs, reconstructs ROI geometry directly from the pixel coordinates, calculates a full set of shape measurements, ranks ROIs by `Circularity`, divides them into 5%-wide rank-based percentile groups, randomly samples example ROIs from each group, and produces detailed PDF and CSV outputs.
...
```text
/home/oltij/Desktop/ROI_circularity_report
```
...
## Focal ranking metric — Circularity
...
Higher values generally indicate shapes closer to a compact circle under this pixel-edge perimeter definition. Values are calculated from the ROI pixel mask itself.
...
│   └── Circularity/
│       ├── PV_Circularity_report.pdf
│       ├── PV_Circularity_selected_ROIs.csv
│       └── PV_Circularity_all_ROIs.csv
├── Hoescht/
│   ├── Hoescht_all_ROI_measurements.csv
│   └── Circularity/
│       ├── Hoescht_Circularity_report.pdf
│       ├── Hoescht_Circularity_selected_ROIs.csv
│       └── Hoescht_Circularity_all_ROIs.csv
└── LHX6/
    ├── LHX6_all_ROI_measurements.csv
    └── Circularity/
        ├── LHX6_Circularity_report.pdf
        ├── LHX6_Circularity_selected_ROIs.csv
        └── LHX6_Circularity_all_ROIs.csv
```

Because `Circularity` contains no spaces, the metric folder and filename component are exactly `Circularity`.
...
```text
<Channel>/Circularity/<Channel>_Circularity_report.pdf
```

For example:

```text
PV/Circularity/PV_Circularity_report.pdf
```
...
```text
PV/Circularity/PV_Circularity_all_ROIs.csv
```
...
```text
PV/Circularity/PV_Circularity_selected_ROIs.csv
```
...
```bash
python your_circularity_report_script.py
```
...
ROI Circularity report script
        ...
        +--> per-channel Circularity PDF
        +--> per-channel Circularity all-ROI CSV
        +--> per-channel Circularity selected-ROI CSV
```
