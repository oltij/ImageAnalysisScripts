...
12. Run colocalization QC and preserve the marker↔nucleus HDF5 mappings
13. Export nuclear-positive marker-cell pixel tables
14. Colocalize nuclear-positive marker cells with other marker cells
15. Run marker↔marker QC, population summaries, organoid-area summaries, and all-nuclear-cell classification
```
...
## True all-nuclear-cell summary for a marker↔marker run
...
└── B ↔ C
     │
     v
MARKER ↔ MARKER QC / SUMMARY
│
├── matched / A-only / B-only metrics
├── unmatched-object QC
├── optional organoid-area coverage
├── recover canonical nuclear IDs from source marker↔nuclear HDF5 files
└── classify all nuclei as A+/B+, A+/B-, A-/B+, or A-/B-
     │
     v
FINAL PAIRWISE + ALL-CELL BIOLOGICAL RELATIONSHIPS
```
