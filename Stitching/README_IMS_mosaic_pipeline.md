...
Optional-but-required for the illumination-correction stage:
...
## Known marker mismatch in this uploaded version

There is an important implementation mismatch in the uploaded source.

The Python wrapper checks for:

```text
BIGSTITCHER_INTENSITY_ADJUSTMENT_START
```

but the generated `bigstitcher_macro()` shown in this version explicitly prints:
...
and does not explicitly print:

```text
BIGSTITCHER_INTENSITY_ADJUSTMENT_START
```

before the `Adjust Intensities` call.

Therefore, unless that exact START text is emitted elsewhere by the Fiji/BigStitcher environment, the post-run Python check can raise:

```text
BigStitcher intensity-adjustment stage was not reached for <channel>
```

even after the macro has run.

A direct code-level fix would be to insert:
...
or to remove/change the corresponding Python-side START-marker check.

This README documents the uploaded implementation as-is; it does not silently assume the missing marker is present.
...
The current pipeline cleans previous pipeline-generated acquisition results by default, so this flag is not the main mechanism controlling reruns.
...
This is retained/documented as an intent flag.

The script already retains exported inputs by default.
...
logical ZYX TIFF export
   |
   +--> optional trailing zero-padding trim
   |
   v
BaSiC correction
   |
